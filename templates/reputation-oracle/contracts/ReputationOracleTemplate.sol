...
import "@aragon/apps-token-manager/contracts/WhitelistOracle.sol";

contract ReputationOracleTemplate is BaseTemplate {
        ...
        public
    {
        ...
        TokenManager tokenManager = _installCustomTokenManagerApp(dao, token, TOKEN_TRANSFERABLE, TOKEN_MAX_PER_ACCOUNT);
        WhitelistOracle oracle = _installOracleApp(dao);
        Voting voting = _installVotingApp(dao, token, _votingSettings[0], _votingSettings[1], _votingSettings[2]);
        _createPermissionForTemplate(acl, tokenManager, tokenManager.SET_ORACLE());
        tokenManager.setOracle(oracle);
        _removePermissionFromTemplate(acl, tokenManager, tokenManager.SET_ORACLE());
        ...
        _createCustomOraclePermissions(acl, oracle,voting);
    ...
    function _createCustomOraclePermissions(ACL _acl, WhitelistOracle _oracle, Voting _voting) internal {
        _acl.createPermission(_voting, _oracle, _oracle.ADD_SENDER_ROLE(), _voting);
        _acl.createPermission(_voting, _oracle, _oracle.REMOVE_SENDER_ROLE(), _voting);
    }
    ...
    function _createCustomTokenManagerPermissions(ACL _acl, TokenManager _tokenManager, Voting _voting) internal {
        ...
        _acl.createPermission(_voting, _tokenManager, _tokenManager.SET_ORACLE(), _voting);
    }

    function _installOracleApp(Kernel _dao) internal returns (WhitelistOracle) {
        WhitelistOracle oracle = WhitelistOracle(_installNonDefaultApp(_dao, apmNamehash("oracle")));
        oracle.initialize(new address[](0));
        return oracle;
    }
