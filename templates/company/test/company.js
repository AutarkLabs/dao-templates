...
contract('Company', ([_, owner, holder1, holder2, someone]) => {
  ...
  const newInstance = (...params) => {
    ...
    const newInstanceFn = CompanyTemplate.abi.find(({ name, inputs }) => name === 'newInstance' && inputs.length === params.length)
  ...
  context('creating instances with a single transaction', () => {
    context('when the creation fails', () => {
      ...
      it('reverts when no holders were given', async () => {
        await assertRevert(template.newTokenAndInstance(TOKEN_NAME, TOKEN_SYMBOL, randomId(), [], [], VOTING_SETTINGS, FINANCE_PERIOD, USE_AGENT_AS_VAULT), 'COMPANY_EMPTY_HOLDERS')
      })
  ...
  context('creating instances with separated transactions', () => {
    context('when the creation fails', () => {
      ...
      context('when there was a token created', () => {
        ...
        it('reverts when no holders were given', async () => {
          await assertRevert(newInstance(randomId(), [], [], VOTING_SETTINGS, FINANCE_PERIOD, USE_AGENT_AS_VAULT), 'COMPANY_EMPTY_HOLDERS')
        })
